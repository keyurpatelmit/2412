...
<?php if(canaccess("city","edit_access") != 'true'){ echo "<script>window.location.href ='".site_url()."admin'</script>";}?>
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="panel panel-color panel-inverse">
                    ...
                    <form name="city_edit" id="city_edit" method="post"  action='<?= base_url("admin/city/edit/{$row->id}"); ?>' enctype="multipart/form-data">
              ...
              <div class="row">
                            ...
                            <select data-live-search="true" class="form-control selectpicker" data-style="btn-white" required="required" name="state_id" id="state_id">
                                        <option value="">Select state</option>
                                           <?php foreach ($states as $key => $state): ?>
                                                <option  <?php if($state['id'] == $row->state_id){ echo 'selected';} ?> value="<?= $state['id'] ?>"><?= $state['state'] ?></option>        
                                        <?php endforeach ?>
                                    </select>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" required="required" name="city" id="city" class="form-control" placeholder="City" value="<?=  $row->city; ?>">
                        </div>
                    </div>
                    <div class="col-md-12">
