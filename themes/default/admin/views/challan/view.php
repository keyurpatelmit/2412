		...
		<div class="row">
	        <div class="col-sm-12">
	            ...
	            <div class="pull-right">
	            	...
	            	<a onclick="formSubmit('excel')" href="<?= base_url("admin/challan/add_challan_form"); ?>" id="unset_button"><button type="button" id="add" class="btn btn-purple waves-effect waves-light">Add New</button></a>
	            	
	            	<?php } ?>		

	            	<a href="<?= base_url("admin/challan"); ?>/unset_session_value" id="unset_button"><button type="button" class="btn btn-primary m-r-5" status="none"><span class="glyphicon glyphicon-refresh"></span></button></a>	
	            </div>
		      ...
		      <div class="panel panel-default">
		         <div class="panel-body">
		            <div class="fixed-table-toolbar">
		               <form autocomplete="off" name="searchfrom" id="searchfrom" method="post"  action='<?= base_url("admin/challan"); ?>' enctype="multipart/form-data">
            		  	...
            		  	<?php $count = 1; 
                			...
                			foreach ($rows as $row) : ?>
	        					<tr>
									...
									<?php
        								if($row['status'] == 'invoice_pending'){
        									$status_class = 'info';
        								}elseif($row['status'] == 'invoice_done'){
        									$status_class = 'success';
        								}elseif($row['status'] == 'temporary'){
        									$status_class = 'danger';
        								}else{
        									$status_class = ''	;
        								}
        								...
        								<a target="_blank" href='<?= base_url("admin/challan/challan_pdf/{$row['id']}"); ?>'><button class="btn btn-icon btn-xs waves-effect waves-light btn-info" data-toggle="tooltip" data-placement="left" title=""> <i class="fa fa-file-pdf-o"></i> </button></a>
        
        								<?php if(canaccess("city","edit_access")){ ?>
        								 <a href='<?= base_url("admin/challan/edit/{$row['id']}"); ?>'><button class="btn btn-icon btn-xs waves-effect waves-light btn-purple" data-toggle="tooltip" data-placement="left" title="" data-original-title="<?= lang('edit'); ?>"> <i class="fa fa-edit"></i> </button></a>
        								<?php } ?>
        								<?php if(canaccess("city","edit_access")){ ?>
...
<script>
	...
	$(document).ready(function() {
        ...
        $('.sa-params').click(function () {
            ...
            swal({
            	...
            	$.ajax({
            		type: "POST",
            		url: "<?php echo site_url('admin/challan/row_delete')?>/"+id,
    ...
    function get_vendor(id)
	{
	     // AJAX request
	     $.ajax({
	       url: "<?php echo site_url('admin/puchaseorder/get_vendor')?>/",
	       method: 'post',
	       data: {id: id},
	       dataType: 'json',
