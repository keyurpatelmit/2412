...
<div class="row">
    <div class="col-lg-12">
        <div class="panel panel-color panel-inverse">
            ...
            <form method="post" enctype="multipart/form-data" name="row_add" id="row_add"  action='<?= base_url("admin/salesorder/edit/{$row->id}"); ?>'>
             ...
             <input type="hidden" name="total_margin" id="total_margin_amount" value="<?= $row->total_margin; ?>">
            ...
            <div class="row">   
                <div class="col-md-3">
                        <div class="form-group">
                            <label for="quotation_subject">Sales Order no <span class="text-danger">*</span></label>
                            <input readonly="" type="text" name="purchase_no" id="purchase_no" class="form-control" value="<?= $row->sales_order_no ?>">
                        </div>
                </div>
                  ...
                  <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input type="text" name="po_no" id="po_no" class="form-control"  value="<?= $row->po_no ?>">
                    </div>
                </div>
                ...
                <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input readonly="" type="text" name="quotation_no" id="quotation_no" class="form-control" value="<?= $row->quotation_no ?>">
                        </div>
                </div>
            </div>
            ...
            <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input type="text" name="sales_order_date" id="sales_order_date" class="form-control datepicker-autoclose" required="required" value="<?= $this->maahi->date_format($row->sales_order_date); ?>">
                    </div>
            </div>
            ...
            <div class="col-md-3">
                <div class="form-group">
                <label for="contact_email">Purchase order status</label>
             ...
             <div class="col-md-3">
                <div class="form-group">
                    ...
                    <input type="date" name="delivery_date" id="delivery_date" class="form-control" value="<?= $row->delivery_date ?>">
                </div>
            </div>
            ...
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    <input type="text" name="delivery_note" id="delivery_note" class="form-control" value="<?= $row->delivery_note ?>">
                </div>
            </div>

               <div class="col-md-3">
                <div class="form-group">
                    <label for="shipping_address_pdf">Shipping address pdf</label>
                    <input type="text" name="shipping_address_pdf" id="shipping_address_pdf" class="form-control" value="<?= $row->shipping_address_pdf ?>">
                </div>
            </div>
            ...
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    <input type="text" name="payment_terms" id="payment_terms" class="form-control" value="<?= $row->payment_terms ?>" >
                </div>
            </div>
        ...
        <div class="row">
             <div class="col-md-12">
                <div class="form-group">
                    <label for="contact_email">Account note</label>
            ...
            <tr id="remove_item_ui_<?= $inquiry_item['id'] ?>">
                <td>
                 ...
                 <div class="<?php if($inquiry_item['item_type'] != 'service') { echo "hide"; } ?>" id="item_service_ui_<?= $randno ?>">
                        <select data-live-search="true" class="selectpicker form-control" name="edit_item_service[]" id="item_service_<?= $randno ?>" data-style="btn-white" onchange="get_item_price_services(<?= $randno ?>)">
                            <option value="">Select Serivce</option>
                            <?php foreach ($services as $key => $service): ?>
                            <option data-gst="<?= $service['gst'] ?>" data-sid="<?= $service['id'] ?>" data-price="<?= $service['price'] ?>" <?php if($inquiry_item['item_id'] == $service['id']) { echo "selected"; } ?> value="<?= $service['id'] ?>"><?= $service['title'] ?></option>  
                            <?php endforeach ?>     
                        </select>
                    </div>

                     <div class="<?php if($inquiry_item['item_type'] != 'product') { echo "hide"; } ?>" id="item_product_ui_<?= $randno ?>">
                          <select data-live-search="true" class="selectpicker form-control" name="edit_item_product[]" id="item_product_<?= $randno ?>" data-style="btn-white" onchange="get_item_price_product(<?= $randno ?>)">
                            <option value="">Select products</option>
                            <?php foreach ($products as $key => $product): ?>
                                <option  data-purchased_price="<?= $product['purchased_price'] ?>" data-gst="<?= $product['gst'] ?>" data-pid="<?= $product['id'] ?>" data-price="<?= $product['price'] ?>" <?php if($inquiry_item['item_id'] == $product['id']) { echo "selected"; } ?> value="<?= $product['id'] ?>"><?= $product['product_name'] ?></option>  
                            <?php endforeach ?>     
                        </select>
                     </div>
                </td>
                 ...
                 <td>
                     <select data-live-search="true" class="selectpicker form-control" name="edit_vendor_id[]" id="vendor_id_<?= $randno ?>" data-style="btn-white">
                        <option>Select vendor</option>
                        <?php foreach ($vendors as $key => $vendor): ?>
                            <option <?php if(@$inquiry_item['vendor_id'] == $vendor['id']) { echo "selected"; } ?> value="<?= $vendor['id'] ?>"><?= $vendor['name'] ?> </option>  
                        <?php endforeach ?>     
                    </select>
                </td>

                 <td><input type="text" name="edit_item_qty[]" id="item_qty_<?= $randno ?>" class="form-control" onkeyup="get_item_total(<?= $randno ?>)" value="<?= $inquiry_item['item_qty'] ?>"></td>
                <td><input type="text total_count" name="edit_item_price[]" id="item_price_<?= $randno ?>" class="form-control " onkeyup="get_item_total(<?= $randno ?>)" value="<?= $inquiry_item['item_price'] ?>">
                
                <input type="hidden" name="edit_item_gst[]" id="item_gst_<?= $randno ?>" class="form-control" value="<?= $inquiry_item['item_gst'] ?>">

                <input type="hidden" name="item_total[]" id="item_total_<?= $randno ?>" class="form-control total_item_price" value="<?= $inquiry_item['item_price'] * $inquiry_item['item_qty'] ?>">
                </td>
                 <td><input type="text" name="edit_purchase_item_price[]" id="purchase_item_price_<?= $randno ?>" class="form-control" value="<?= $inquiry_item['purchase_item_price'] ?>" onkeyup="get_item_total(<?= $randno ?>)"></td>

                <td><input type="text" name="edit_margin_item_price[]"  id="margin_item_price_<?= $randno ?>" class="form-control total_margin_item_price" value="<?= $inquiry_item['margin_item_price'] ?>"></td>
        ...
        <?php $randno = rand(); ?>
                     ...
                     <td>
                        <div class="hide" id="item_service_ui_<?= $randno ?>">
                            <select data-live-search="true" class="selectpicker form-control" name="item_service[]" id="item_service_<?= $randno ?>" data-style="btn-white" onchange="get_item_price_services(<?= $randno ?>)">
                                <option value="">Select Serivce</option>
                                <?php foreach ($services as $key => $service): ?>
                                <option data-sid="<?= $service['id'] ?>" data-price="<?= $service['price'] ?>" value="<?= $service['id'] ?>"><?= $service['title'] ?></option>  
                                <?php endforeach ?>     
                            </select>
                        </div>
            ...
            <tr>
                <td colspan="2"></td>
                <td><b>Total </b></td>
                <td><b><span id="total_price"><?= $row->total_amount; ?></span> /-</b></td>
                <td>-</td>
                <td><b><span id="total_margin_price"><?= $row->total_margin; ?></span> /-</b></td>
        ...
        <div class="row">
         ...
         <table class="table">
            ...
            <tbody id="term_condition_ui">
                ...
                <?php foreach ($quotation_terms as $key => $quotation_term): ?>
                <?php $randno = rand(); ?>
                <tr id="remove_po_terms_row_<?= $quotation_term['id'] ?>">
                    <td>
                     <input type="hidden" name="update_terms_id[]" id="update_terms_id" class="form-control" value="<?= $quotation_term['id'] ?>">
                        ...
                        <?php foreach ($terms_condition as $key => $termscondition): ?>
                            <option <?php if($termscondition['id'] == $quotation_term['term_id']){ echo "selected"; } ?>  value="<?= $termscondition['id'] ?>"><?= $termscondition['title'] ?></option>
                        <?php endforeach ?>
                        </select>
                    </td>

                    <td>
                        <textarea name="edit_term_condition_txt[]" id="term_condition_txt_<?= $randno ?>" class="form-control term_condition_txt"><?= $quotation_term['item_description'] ?></textarea>
                    </td>
                     <td>
                    <a class="sa-params-terms" href='javascript:void(0)' id="<?= $quotation_term['id'] ?>"><button class="btn btn-danger" type="button"><i class="fa fa-minus-circle"></i> Remove </button>
                    </a></td>
                </tr>
                <?php endforeach ?>
                <?php endif ?>
        ...
        <div class="panel-footer">
            <div class="clearfix">
                ...
                <a href="<?= base_url('admin/salesorder'); ?>"><button type="button" class="btn btn-inverse waves-effect waves-light pull-right" data-dismiss="modal" aria-hidden="true">Close</button></a>
            </div>
        </div>
    </form>
    </div>
    ...
    $(document).ready(function() {
        ...
        $('.sa-params-terms').click(function () {
            ...
            swal({
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo site_url('admin/salesorder/remove_salesorder_terms')?>/"+id,
        ...
        $('.sa-params').click(function () {
            ...
            swal({
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo site_url('admin/salesorder/remove_items')?>/"+id,
                    success: function(data) {
                        swal({
                            title: 'Deleted!',
                            text: 'Successfully Record deleted.',
