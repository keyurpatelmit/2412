...
<?php if(canaccess("component","view_access") != 'true'){ echo "<script>window.location.href ='".site_url()."admin'</script>";}?>
		...
		<div class="row">
			<div class="col-sm-12">
				<div class="pull-right">
					...
					<?php if(canaccess("component","create_access")) { ?> 
					...
					<a href="<?= base_url("admin/component"); ?>/unset_session_value" id="unset_button"><button type="button" class="btn btn-primary m-r-5" status="none"><span class="glyphicon glyphicon-refresh"></span></button></a>	
				</div>
			...
			<div class="panel panel-default">
				<div class="panel-body">
					<div class="fixed-table-toolbar">
						<form autocomplete="off" name="searchfrom" id="searchfrom" method="post"  action='<?= base_url("admin/component"); ?>' enctype="multipart/form-data">
							...
							<div class="pull-left search margin m-r-15">
								...
								<input class="form-control" type="text" placeholder="Any Text.." name="component_s_title" id="component_s_title" value="<?php if($this->session->userdata('component_s_title')){echo $this->session->userdata('component_s_title'); 
							} 
						...
						<div class="pull-left search margin m-r-15">
							...
							<select id="component_s_status" name="component_s_status" class="form-control">
								<option value="" >Select Status</option>
								<option 
								<?php 
								if($this->session->userdata('component_s_status')) 
								{
									if($this->session->userdata('component_s_status') == 'active')
									{
										echo "selected"; 
									}				                            	
								}
								...
								if($this->session->userdata('component_s_status')) 
								{
									if($this->session->userdata('component_s_status') == 'inactive')
									{
										echo "selected"; 
									}				                            	
								}
					...
					<?php if(!empty($rows)) {  ?>
						...
						foreach ($rows as $row) : ?>
							<tr>
								...
								<td>
									<?php if(canaccess("component","edit_access")){ ?>
										<a href='<?= base_url("admin/component/edit/{$row['id']}"); ?>'><button class="btn btn-icon btn-xs waves-effect waves-light btn-purple" data-toggle="tooltip" data-placement="left" title="" data-original-title="<?= lang('edit'); ?>"> <i class="fa fa-edit"></i> </button></a>
									<?php } ?>
									<?php if(canaccess("component","delete_access")){ ?>
	...
	$(document).ready(function() {
		...
		$('.sa-params').click(function () {
			...
			swal({
            	...
            	$.ajax({
            		type: "POST",
            		url: "<?php echo site_url('admin/component/row_delete')?>/"+id,
