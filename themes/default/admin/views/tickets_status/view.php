...
<?php if(canaccess("tickets_status","view_access") != 'true'){ echo "<script>window.location.href ='".site_url()."admin'</script>";}?>
<div class="wrapper uppercase_text">
    <div class="container">
		<div class="row">
	        <div class="col-sm-12">
	            ...
	            <div class="pull-right">
	            	...
	            	<?php if(canaccess("tickets_status","create_access")) { ?> 
	            	...
	            	<a href="<?= base_url("admin/tickets_status"); ?>/unset_session_value" id="unset_button"><button type="button" class="btn btn-primary m-r-5" status="none"><span class="glyphicon glyphicon-refresh"></span></button></a>	
	            </div>
	    ...
	    <?php if(empty($_REQUEST)) { ?>
	    	<?php if(!empty($this->session->userdata('tickets_status_serach_data'))) { ?>
	    		style="display: block;"
	    	<?php }else{ ?>
				style="display: none;"
	    	<?php  } ?>
		      ...
		      <div class="panel panel-default">
		         <div class="panel-body">
		            <div class="fixed-table-toolbar">
		               <form autocomplete="off" name="searchfrom" id="searchfrom" method="post"  action='<?= base_url("admin/tickets_status"); ?>' enctype="multipart/form-data">
		                 ...
		                 <input type="hidden" name="SearchValue" id="SearchValue">
		                  ...
		                  <div class="pull-left search margin m-r-15">
		                     ...
		                     <input class="form-control" type="text" placeholder="Any Text.." name="tickets_status_s_title" id="tickets_status_s_title" value="<?php if($this->session->userdata('tickets_status_s_title')){echo $this->session->userdata('tickets_status_s_title'); 
		                     			} 
		                  ...
		                  <div class="pull-left search margin m-r-15">
		                     ...
		                     <select id="tickets_status_s_status" name="tickets_status_s_status" class="form-control">
		                        <option value="" >Select Status</option>
		                        <option 
	              					<?php 
	              					if($this->session->userdata('tickets_status_s_status')) 
	              					{
										if($this->session->userdata('tickets_status_s_status') == 'active')
		              					{
		              						echo "selected"; 
		              					}				                            	
		                            }
		                        ...
		                        <option 
	              					...
	              					if($this->session->userdata('tickets_status_s_status')) 
	              					{
										if($this->session->userdata('tickets_status_s_status') == 'inactive')
		              					{
		              						echo "selected"; 
		              					}				                            	
		                            }
            		  ...
            		  <?php $count = 1; 
                			...
                			foreach ($rows as $row) : ?>
	        					<tr>
	        						<td><?= $count++; ?></td>
									<td><?= $row['title']; ?></td>
									<td><span class="label" style="width: 50px;height: 50px;background: <?= $row['color']; ?>"><?= $row['color']; ?></span></td>
									<td><?= ucfirst($row['is_default']); ?></td>
									<td><?= ucfirst($row['is_last']); ?></td>
									<td><span class="label label-<?php echo ($row['status'] == 'active') ? 'success' : 'danger'?>"><?= ucfirst($row['status']); ?></span></td>
									<td>
								<?php if(canaccess("tickets_status","edit_access")){ ?>
								 <a href='<?= base_url("admin/tickets_status/edit/{$row['id']}"); ?>'><button class="btn btn-icon btn-xs waves-effect waves-light btn-purple" data-toggle="tooltip" data-placement="left" title="" data-original-title="<?= lang('edit'); ?>"> <i class="fa fa-edit"></i> </button></a>
								<?php } ?>
								<?php if(canaccess("tickets_status","delete_access")){ ?>
	              ...
	              <div class="row">
	              	...
	              	<div class="col-md-6">
	              		<div class="form-group">
	              			...
	              			<input type="color" required="required" name="color" id="color" class="form-control">
	              		</div>
	              	</div>
	              	<div class="col-md-6">
                        <div class="form-group checkbox checkbox-danger" style="margin-top: 30px;">
                            <input type="checkbox" name="is_default" value="yes" id="status1">
                            <label for="status1"><?= lang('is_default'); ?></label>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group checkbox checkbox-danger" style="margin-top: 30px;">
                            <input type="checkbox" name="is_last" value="yes" id="status2">
                            <label for="status2"><?= lang('is_last'); ?></label>
                        </div>
                    </div>
...
<script>
	...
	$(document).ready(function() {
		$('#row_add').on('submit',function(e) {
			...
			if($('#title').val() == '' || $('#color').val() == '')  { return false; }
			$("#save").prepend(' <i class = "fa fa-spinner fa-spin"></i> ').prop( "disabled", true );

			$.ajax({
				url: "<?php echo site_url('admin/tickets_status/add_row')?>/",
				type: "POST",
        ...
        $('.sa-params').click(function () {
            ...
            swal({
            	...
            	$.ajax({
            		type: "POST",
            		url: "<?php echo site_url('admin/tickets_status/row_delete')?>/"+id,
