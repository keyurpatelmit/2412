...
<?php if(canaccess("state","view_access") != 'true'){ echo "<script>window.location.href ='".site_url()."admin'</script>";}?>
	        ...
	        <div class="col-sm-12">
	            ...
	            <div class="pull-right">
	            	...
	            	<button type="button" class="btn btn-primary" status="none" id="advanced_search_btn"><span class="glyphicon glyphicon-search m-r-5 "></span></button>
	            	 <?php if(canaccess("state","create_access")) { ?> 
	            	...
	            	<a href="<?= base_url("admin/state"); ?>/unset_session_value" id="unset_button"><button type="button" class="btn btn-primary m-r-5" status="none"><span class="glyphicon glyphicon-refresh"></span></button></a>	
	            </div>
		      ...
		      <div class="panel panel-default">
		         <div class="panel-body">
		            <div class="fixed-table-toolbar">
		               <form autocomplete="off" name="searchfrom" id="searchfrom" method="post"  action='<?= base_url("admin/state"); ?>' enctype="multipart/form-data">
		                 ...
		                 <input type="hidden" name="SearchValue" id="SearchValue">
		                  ...
		                  <div class="pull-left search margin m-r-15">
		                     ...
		                     <input class="form-control" type="text" placeholder="Any Text.." name="state_s_state" id="state_s_state" value="<?php if(isset($_REQUEST['state_s_state'])) 
		                     			{ 
		                     				echo $_REQUEST['state_s_state']; 
		                     			} 
		                     			else 
		                     			{ 
		                     				if($this->session->userdata('state_s_state')) 
		                     				{ 
		                     					echo $this->session->userdata('state_s_state'); 
		                     				} 
		                     			}
             					?>">
		                  </div>
		                  <div class="pull-left search margin m-r-15">
		                     ...
		                     <select id="state_s_status" name="state_s_status" class="form-control">
		                        <option value="" >Select Status</option>
		                        <option 
		                        <?php 
	              					if(isset($_REQUEST['state_s_status']))
	              					{
	              					if($_REQUEST['state_s_status'] == 'active')
	              					{
	              						echo "selected"; 
	              					}	
	              					}
	              					...
	              					<?php 
	              					if($this->session->userdata('state_s_status')) 
	              					{
										if($this->session->userdata('state_s_status') == 'active')
		              					{
		              						echo "selected"; 
		              					}				                            	
		                            }
		                        ...
		                        <?php 
	              					if(isset($_REQUEST['state_s_status']))
	              					{
	              					if($_REQUEST['state_s_status'] == 'inactive')
	              					{
	              						echo "selected"; 
	              					}	
	              					}
	              					...
	              					if($this->session->userdata('state_s_status')) 
	              					{
										if($this->session->userdata('state_s_status') == 'inactive')
		              					{
		              						echo "selected"; 
		              					}				                            	
		                            }
            		  ...
            		  <?php $count = 1; 
                			...
                			foreach ($rows as $row) : ?>
	        					<tr>
									...
									<td>
								<?php if(canaccess("state","edit_access")){ ?>
								 <a href='<?= base_url("admin/state/edit/{$row['id']}"); ?>'><button class="btn btn-icon btn-xs waves-effect waves-light btn-purple" data-toggle="tooltip" data-placement="left" title="" data-original-title="<?= lang('edit'); ?>"> <i class="fa fa-edit"></i> </button></a>
								<?php } ?>
									<?php if(canaccess("state","edit_access")){ ?>
...
<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
        	<form name="state_add" id="state_add" method="post" enctype="multipart/form-data">
	              ...
	              <div class="row">
		              	...
		              	<div class="col-md-6">
		              		<div class="form-group">
		              			...
		              			<input type="text" required="required" name="state" id="state" class="form-control" placeholder="state">
		              		</div>
	              	</div>
	              </div>
	            </div>
...
<script>
	...
	$(document).ready(function() {
		$('#state_add').on('submit',function(e) {
			...
			$.ajax({
				url: "<?php echo site_url('admin/state/add_row')?>/",
				type: "POST",
        ...
        $('.sa-params').click(function () {
            ...
            swal({
            	...
            	$.ajax({
            		type: "POST",
            		url: "<?php echo site_url('admin/state/row_delete')?>/"+id,
