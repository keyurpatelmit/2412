            ...
            <div class="col-sm-12">
                ...
                <div class="pull-right"> 
                    ...
                    <a onclick="formSubmit('excel')" href="javascript:void();" id="unset_button"><button type="button" class="btn btn-default" status="none"><span class="glyphicon glyphicon-download-alt"></span> Export</button></a>
                        <?php } ?>      
                    <a href="<?= base_url("admin/customer_address"); ?>/unset_session_value" id="unset_button"><button type="button" class="btn btn-primary m-r-5" status="none"><span class="glyphicon glyphicon-refresh"></span></button></a> 
                </div>
                ...
                <div class="panel panel-default">
                    <div class="panel-body">
                        <div class="fixed-table-toolbar">
                            <form autocomplete="off" name="searchfrom" id="searchfrom" method="post"  action='<?= base_url("admin/customer_address"); ?>' enctype="multipart/form-data">
                                    ...
                                    <button onclick="formSubmit('search')" type="submit" name="filter" id="filter" class="btn btn-success" value="filter">Search</button>
                                     ...
                                     <div class="pull-left search margin m-r-15">
                                         <div class="searchdatetitle">Customer</div>
                                          <select   data-live-search="true" name="customer_id" id="customer_id" class="selectpicker form-control"  data-style="btn-white">
                                          <option value="">Select Customer </option>
                                          <?php foreach ($customers as $key => $customer): ?>
                                          <option 
                                            <?php 
                                             if($this->session->userdata('customer_id')){
                                                if($customer['id'] == $this->session->userdata('customer_id')) { echo "selected"; }
                                              }
                                            ?>
                    ...
                    <?php 
                        ...
                        {  ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?= $this->sam->get_real_value('zyd_state','id',$row['cust_state_id'],'state'); ?> </td>
                                <td><?= $this->sam->get_real_value('zyd_city','id',$row['cust_city_id'],'city'); ?> </td>
                                <td><?= $row['cust_zip_code']; ?></td>
                                <td><span class="label label-<?php echo ($row['status'] == 'active') ? 'success' : 'danger'?>"><?= ucfirst($row['status']); ?></span></td>
                                <td>
                                    <button class="btn btn-icon btn-xs waves-effect waves-light btn-info"  data-toggle="farmer_meeting_modal" data-placement="right" title="" data-original-title="<?= lang('Farmer meeting'); ?>" onclick="view_details_inmodel('<?= $row['customer_id'] ?>')"> <i class="fa fa-eye"></i> </button>
                                        <?php if(canaccess("services","edit_access")){ ?>
                                    <a href='<?= base_url("admin/customer_address/edit/{$row['id']}"); ?>'><button class="btn btn-icon btn-xs waves-effect waves-light btn-purple" data-toggle="tooltip" data-placement="left" title="" data-original-title="<?= lang('edit'); ?>"> <i class="fa fa-edit"></i> </button></a>
                                    <?php } ?>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form name="update_zm_ganit_form" id="update_zm_ganit_form" method="post" enctype="multipart/form-data">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                         <h4 class="modal-title" id="myLargeModalLabel">Customer Address View</h4>
                    </div>
                            <div class="modal-body">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#add_form').on('submit',function(e) {
            ...
            $.ajax({
                url: "<?php echo site_url('admin/customer_address/add_row')?>/",
                type: "POST",
        ...
        $('.sa-params').click(function () {
            ...
            swal({
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo site_url('admin/customer_address/row_delete')?>/"+id,
