...
<?php if(canaccess("product_sub_category","view_access") != 'true'){ echo "<script>window.location.href ='".site_url()."admin'</script>";}?>
<div class="wrapper uppercase_text">
    <div class="container">
		<div class="row">
	        <div class="col-sm-12">
	            ...
	            <div class="pull-right">
	            	...
	            	<?php if(canaccess("product_sub_category","create_access")) { ?> 
	            	...
	            	<a href="<?= base_url("admin/product_sub_category"); ?>/unset_session_value" id="unset_button"><button type="button" class="btn btn-primary m-r-5" status="none"><span class="glyphicon glyphicon-refresh"></span></button></a>	
	            </div>
	    ...
	    <?php if(empty($_REQUEST)) { ?>
	    	<?php if(!empty($this->session->userdata('product_sub_category_serach_data'))) { ?>
	    		style="display: block;"
	    	<?php }else{ ?>
				style="display: none;"
	    	<?php  } ?>
		      ...
		      <div class="panel panel-default">
		         <div class="panel-body">
		            <div class="fixed-table-toolbar">
		               <form autocomplete="off" name="searchfrom" id="searchfrom" method="post"  action='<?= base_url("admin/product_sub_category"); ?>' enctype="multipart/form-data">
		                 ...
		                 <input type="hidden" name="SearchValue" id="SearchValue">
		                  ...
		                  <div class="pull-left search margin m-r-15">
		                     ...
		                     <input class="form-control" type="text" placeholder="Any Text.." name="product_sub_category_s_title" id="product_sub_category_s_title" value="<?php if($this->session->userdata('product_sub_category_s_title')){echo $this->session->userdata('product_sub_category_s_title'); 
		                     			} 
		                  ...
		                  <div class="pull-left search margin m-r-15">
		                     ...
		                     <select id="product_sub_category_s_status" name="product_sub_category_s_status" class="form-control">
		                        <option value="" >Select Status</option>
		                        <option 
	              					<?php 
	              					if($this->session->userdata('product_sub_category_s_status')) 
	              					{
										if($this->session->userdata('product_sub_category_s_status') == 'active')
		              					{
		              						echo "selected"; 
		              					}				                            	
		                            }
		                        ...
		                        <option 
	              					...
	              					if($this->session->userdata('product_sub_category_s_status')) 
	              					{
										if($this->session->userdata('product_sub_category_s_status') == 'inactive')
		              					{
		              						echo "selected"; 
		              					}				                            	
		                            }
    		  ...
    		  <?php $count = 1; 
        			...
        			foreach ($rows as $row) : ?>
    					<tr>
							...
							<td>
							<?php if(canaccess("product_sub_category","edit_access")){ ?>
							 <a href='<?= base_url("admin/product_sub_category/edit/{$row['id']}"); ?>'><button class="btn btn-icon btn-xs waves-effect waves-light btn-purple" data-toggle="tooltip" data-placement="left" title="" data-original-title="<?= lang('edit'); ?>"> <i class="fa fa-edit"></i> </button></a>
							<?php } ?>
							<?php if(canaccess("product_sub_category","delete_access")){ ?>
...
<script>
	...
	$(document).ready(function() {
		$('#row_add').on('submit',function(e) {
			...
			$.ajax({
				url: "<?php echo site_url('admin/product_sub_category/add_row')?>/",
				type: "POST",
        ...
        $('.sa-params').click(function () {
            ...
            swal({
            	...
            	$.ajax({
            		type: "POST",
            		url: "<?php echo site_url('admin/product_sub_category/row_delete')?>/"+id,
