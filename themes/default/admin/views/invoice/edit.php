...
<?php if(canaccess("invoice","edit_access") != 'true'){ echo "<script>window.location.href ='".site_url()."admin'</script>";}?>
...
<div class="row">
    <div class="col-lg-12">
        <div class="panel panel-color panel-inverse">
            ...
            <form method="post" enctype="multipart/form-data" name="row_add" id="row_add"  action='<?= base_url("admin/invoice/edit/{$row->id}"); ?>'>
               ...
               <div class="row">   
                <div class="col-md-3">
                        <div class="form-group">
                            <label for="quotation_subject">Invoice No <span class="text-danger">*</span></label>
                            <input readonly="" type="text" name="invoice_no" id="invoice_no" class="form-control" value="<?= $row->invoice_no ?>">
                        </div>
                </div>
            </div>
              <div class="row">
            ...
            <div class="col-md-3">
                    <div class="form-group">
                        <label for="name">Invoice date <span class="text-danger">*</span></label>
                        <input type="text" name="invoice_date" id="invoice_date" class="form-control datepicker-autoclose" required="required" value="<?= $this->maahi->date_format($row->invoice_date); ?>">
                    </div>
            </div>
            ...
            <label for="contact_email">Inquiry status</label>
                <select class="form-control" name="status" id="status" >
                    ...
                    <option <?php if($row->status == 'open') { echo "selected"; }  ?> value="open">new</option>
                    <option <?php if($row->status == 'close') { echo "selected"; }  ?> value="close">new</option>
                </select>
            </div>
            </div>
        ...
        <?php if (!empty($inquiry_items)): ?>
            ...
            <tr id="remove_item_ui_<?= $inquiry_item['id'] ?>">
                 ...
                 <td>
                    ...
                    <div class="<?php if($inquiry_item['item_type'] != 'service') { echo "hide"; } ?>" id="item_service_ui_<?= $randno ?>">
                        <select data-live-search="true" class="selectpicker form-control" name="edit_item_service[]" id="item_service_<?= $randno ?>" data-style="btn-white" onchange="get_item_price_services(<?= $randno ?>)">
                            ...
                            <option data-vendor_id="<?= $service['vendor_id'] ?>" data-price="<?= $service['price'] ?>" <?php if($inquiry_item['item_id'] == $service['id']) { echo "selected"; } ?> value="<?= $service['id'] ?>"><?= $service['title'] ?></option>  
                            <?php endforeach ?>     
                        </select>
                    </div>
                     ...
                     <div class="<?php if($inquiry_item['item_type'] != 'product') { echo "hide"; } ?>" id="item_product_ui_<?= $randno ?>">
                          <select data-live-search="true" class="selectpicker form-control" name="edit_item_product[]" id="item_product_<?= $randno ?>" data-style="btn-white" onchange="get_item_price_product(<?= $randno ?>)">
                            ...
                            <?php foreach ($products as $key => $product): ?>
                                <option data-vendor_id="<?= $product['vendor_id'] ?>" data-price="<?= $product['price'] ?>" <?php if(@$inquiry_item['item_id'] == $product['id']) { echo "selected"; } ?> value="<?= $product['id'] ?>"><?= $product['product_name'] ?></option>  
                            <?php endforeach ?>     
                        </select>
                     </div>
                </td>
                ...
                <td><textarea name="edit_item_description[]" id="item_description" class="form-control item_description"><?= $inquiry_item['item_description'] ?></textarea></td>

                <td><textarea name="edit_item_remark[]" id="item_remark_<?= $randno ?>" class="form-control "><?= $inquiry_item['item_remark'] ?></textarea></td>
        ...
        <?php $randno = rand(); ?>
                ...
                <tr>
                     ...
                     <td>
                        <div class="hide" id="item_service_ui_<?= $randno ?>">
                            <select data-live-search="true" class="selectpicker form-control" name="item_service[]" id="item_service_<?= $randno ?>" data-style="btn-white" onchange="get_item_price_services(<?= $randno ?>)">
                                <option value="">Select Serivce</option>
                                <?php foreach ($services as $key => $service): ?>
                                <option data-gst="<?= $service['gst'] ?>" data-price="<?= $service['price'] ?>" value="<?= $service['id'] ?>"><?= $service['title'] ?></option>  
                                <?php endforeach ?>     
                            </select>
                        </div>
                         ...
                         <div class="hide" id="item_product_ui_<?= $randno ?>">
                              <select data-live-search="true" class="selectpicker form-control" name="item_product[]" id="item_product_<?= $randno ?>" data-style="btn-white" onchange="get_item_price_product(<?= $randno ?>)">
                                <option value="">Select products</option>
                                <?php foreach ($products as $key => $product): ?>
                                <option data-gst="<?= $product['gst'] ?>" data-price="<?= $product['price'] ?>" value="<?= $product['id'] ?>"><?= $product['product_name'] ?></option>  
                                <?php endforeach ?>    
                            </select>
                         </div>
                    </td>
                    ...
                    <td><textarea name="item_description[]" id="item_description" class="form-control item_description"></textarea></td>

                    <td><textarea name="item_remark[]" id="item_remark" class="form-control "></textarea></td>

                   <!--  <td><input type="text" name="item_remark[]" id="item_remark_<?= $randno ?>" class="form-control" ></td> -->
        ...
        <div class="row"><div class="col-md-6" id="header_ui"><h3>Activity to be done by support department</h3></div></div>
    ...
    <tbody id="support_list_ui">
   <?php if (!empty($row->support_dept)): ?>
    
   <?php foreach (json_decode(@$row->support_dept) as $supportdept): ?>

    <tr id="support_forcontactrow">
        <td><input type="text"  name="support_dept[]" id="support_dept_<?= $randno ?>"  class="form-control " value="<?= $supportdept ?>"></td>

        <td><button id="support_closebuttoncontact" class="btn btn-danger" type="button"><i class="fa fa-minus-circle"> </button></td>
    </tr>
    ...
    <?php endif ?>
                ...
                <tr>
                    <td><input type="text"  name="support_dept[]" id="support_dept_<?= $randno ?>" class="form-control "></td>

                    <td><button onclick="add_support_dept()" class="btn btn-success" type="button"><i class="fa fa-plus-circle"> </button></td>
                </tr>
        ...
        <div class="panel-footer">
            <div class="clearfix">
                <a href="<?= base_url('admin/invoice'); ?>"><button type="button" class="btn btn-inverse waves-effect waves-light pull-right" data-dismiss="modal" aria-hidden="true">Close</button></a>
    ...
    $(document).on('click','#support_closebuttoncontact',function() { 
    $(this).closest("#support_forcontactrow").remove();
});

function add_support_dept()
{
    
    var randno = Math.floor((Math.random() * 999999) + 111111);
    $('#support_list_ui').append('<tr id="support_forcontactrow">'+
            '<td><input type="text" name="support_dept[]" id="support_dept_'+randno+'" class="form-control "></td>'+
            '<td>'+
                '<button id="support_closebuttoncontact" class="btn btn-danger" type="button"><i class="fa fa-minus"></i></button>'+
            '<td>'+
        '</tr>');
    ...
    $(document).ready(function() {
        ...
        $('.sa-params').click(function () {
            ...
            swal({
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo site_url('admin/invoice/remove_items')?>/"+id,
...
function add_inquiry_item()
{
    ...
    $('#inquiry_list_ui').append('<tr id="forcontactrow">'+
            ...
            '<td>'+
                '<div class="hide" id="item_service_ui_'+randno+'">'+
                    '<select data-live-search="true" name="item_service[]" id="item_service_'+randno+'" class="selectpicker form-control"  data-style="btn-white"  onchange="get_item_price_services('+randno+')">'+
                        ...
                        <?php foreach($services as $key=>$service) {?>
                        '<option data-gst="<?= $service['gst'] ?>" data-vendor_id="<?= $service['vendor_id'] ?>" data-price="<?= $service['price'] ?>" value="<?= $service['id'] ?>"><?= $service['title'] ?></option>'+
                        <?php } ?>
                     '</select>'+
                 '</div>'+
                 ...
                 '<div class="hide" id="item_product_ui_'+randno+'">'+
                    ...
                    <?php foreach($products as $key=>$product) {?>
                            '<option data-gst="<?= $product['gst'] ?>" data-vendor_id="<?= $product['vendor_id'] ?>" data-price="<?= $product['price'] ?>" value="<?= $product['id'] ?>"><?= $product['product_name'] ?></option>'+
                    <?php } ?>
                 '</select>'+
                 '</div>'+
            ...
            '<td><textarea name="item_remark[]" id="item_remark_'+randno+'" class="form-control" ></textarea></td>'+
        ...
        showTinymic_small('#item_description_'+randno);
}
        ...
        function get_item_price_product(item_randno)
        {
            ...
            var vendor_id = $('#item_product_'+item_randno+' option:selected').data('vendor_id');
            
            $('#item_price_'+item_randno).val(item_price);
            $('#item_gst_'+item_randno).val(item_gst);
            $("#vendor_id_"+item_randno).val(vendor_id);
            $("#vendor_id_"+item_randno).selectpicker('refresh');
        }
    function get_item_price_services(item_randno)
    {
        ...
        var vendor_id = $('#item_service_'+item_randno+' option:selected').data('vendor_id');
        $('#item_price_'+item_randno).val(item_price);
        $('#item_gst_'+item_randno).val(item_gst);
        $("#vendor_id_"+item_randno).val(vendor_id);
        $("#vendor_id_"+item_randno).selectpicker('refresh');
