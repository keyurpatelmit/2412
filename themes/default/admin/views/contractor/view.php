...
<div class="wrapper uppercase_text">
		...
		<div class="row">
	        <div class="col-sm-12">	            
	            ...
	            <div class="pull-right ">
	            	...
	            	<a href="<?= base_url("admin/contractor/"); ?>unset_session_value" id="unset_button"><button type="button" class="btn btn-primary m-r-5" status="none"><span class="glyphicon glyphicon-refresh"></span></button></a>		
	            </div>
 ...
 <div class="row" id="advanced_search_div" 
        <?php if(empty($_REQUEST)) { ?>
            <?php if(!empty($this->session->userdata('contractor_serach_data'))) { ?>
                style="display: block;"
            <?php }else{ ?>
                style="display: none;"
            <?php  } ?>
		      ...
		      <div class="panel panel-default">
		         <div class="panel-body">
		            <div class="fixed-table-toolbar">
		               <form name="searchfrom" id="searchfrom" method="post"  action='<?= base_url("admin/contractor"); ?>' enctype="multipart/form-data">
	                      ...
	                      <div class=" pull-left search margin m-r-15 ">
		                     <div class="searchdatetitle">contractor Name</div>
		                     <input class="form-control" type="text" placeholder="Any Text.." name="contractor_s_name" id="contractor_s_name" value="<?= (isset($_REQUEST['contractor_s_name']) ? $_REQUEST['contractor_s_name'] : '');?>">
		                  </div>
		                  <div class="pull-left search margin m-r-15 ">
		                     <div class="searchdatetitle">Mobile </div>
		                     <input class="form-control" type="text" placeholder="Any Text.." name="contractor_s_phone" id="contractor_s_phone" value="<?= (isset($_REQUEST['contractor_s_phone']) ? $_REQUEST['contractor_s_phone'] : '');?>">
		                  </div>
		                  <div class="pull-left search margin m-r-15 ">
		                     <div class="searchdatetitle">Email Address</div>
		                     <input class="form-control" type="text" placeholder="Any Text.." name="contractor_s_email" id="contractor_s_email" value="<?= (isset($_REQUEST['contractor_s_email']) ? $_REQUEST['contractor_s_email'] : '');?>">
		                  </div>
                		...
                		<?php $count = 1;if($this->uri->segment(4) != ""){ $count = $this->uri->segment(4) + 1; } foreach ($rows as $row) : ?>
	        					<tr>
								    ...
								    <button class="btn btn-icon btn-xs waves-effect waves-light btn-info"  data-toggle="farmer_meeting_modal" data-placement="right" title="" data-original-title="<?= lang('Farmer meeting'); ?>" onclick="view_details_inmodel('<?= $row['id'] ?>')"> <i class="fa fa-eye"></i> </button>
                                     ...
                                     <a href='<?= base_url("admin/contractor/edit/{$row['id']}"); ?>'><button class="btn btn-icon btn-xs waves-effect waves-light btn-purple" data-toggle="tooltip" data-placement="left" title="" data-original-title="<?= lang('edit'); ?>"> <i class="fa fa-edit"></i> </button></a>
                                    <?php } ?>
        ...
        <div class="modal-content">
        	<form name="add_row" id="add_row" method="post" enctype="multipart/form-data">
	            ...
	            <div class="modal-header">
	                ...
	                <h4 class="modal-title" id="myLargeModalLabel"><?= lang('Add Contractor'); ?></h4> 
	            </div>
            	...
            	<div class="col-md-4 ">
                        <div class="form-group">
                            <label for="cust_state"><?= lang('Customers'); ?></label>
                            <select data-live-search="true" name="customer_id[]" id="customer_id" class="selectpicker form-control"  data-style="btn-white" onchange="get_single_city()" multiple>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form name="update_zm_ganit_form" id="update_zm_ganit_form" method="post" enctype="multipart/form-data">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                         <h4 class="modal-title" id="myLargeModalLabel">Contractor View</h4>
                    </div>
                            <div class="modal-body">
...
function add_new_contact()
{
	...
	$(document).ready(function() {
		...
		$('#add_row').on('submit',function(e) {
			...
			$.ajax({
				url: "<?php echo site_url('admin/contractor/add_row')?>/",
				type: "POST",
...
$(document).on('click','#remove_field',function() {
    ...
    {
        $.ajax({
        url: "<?php echo site_url('admin/contractor/get_details_inmodel')?>/",
        method: 'post',
        data: {id: id},
        dataType: 'json',
