...
class Products_old extends MY_Controller {
	...
	function __construct()
    {
        ...
        $this->load->model('Mdl_products_old','mdl');
    ...
    public function add_row() 
    {
        ...
        $d->subcategory_ids         = @$_POST['subcategory_ids'];
        ...
        if(count($_POST['attribute_id']) > 0){
            for($i=0;$i<count($_POST['attribute_id']);$i++){  
                ...
                $attr_data->product_id          = $d->id;
                $attr_data->attribute_id        = @$_POST['attribute_id'][$i];
                $attr_data->quantity            = @$_POST['quantity'][$i];
                $attr_data->price               = @$_POST['price'][$i];
                $attr_data->remark              = @$_POST['remark'][$i];
                $attr_data->inserted_time       = date('Y-m-d H:i:s');
                $attr_data->save();
            }
        }
     ...
     public function edit($id) 
     {
        ...
        if($_POST && $_POST['id'] != '') {
            ...
            $this->data['product_categories']       =  $this->maahi->get_active_data('zyd_product_category');
            $this->data['product_sub_categories']   =  $this->maahi->get_single_subcategory_from_category($this->data['row']->category_ids);
            $this->data['attributes'] =  $this->maahi->get_selected_prod_attribute($this->data['row']->category_ids,$this->data['row']->subcategory_ids);
            $this->data['prodact_attributes'] =  $this->mdl->get_product_attributes($this->data['row']->id);
            $this->data['prodact_images'] =  $this->mdl->get_products_img($this->data['row']->id);
            $this->data['vendors']              =  $this->maahi->get_vendor_list();
            $this->data['uoms']             =  $this->maahi->get_active_data('tech_uom');
            $this->data['brands']           =  $this->maahi->get_active_data('zyd_product_brand');
