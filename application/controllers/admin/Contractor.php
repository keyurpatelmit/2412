...
class Contractor extends MY_Controller {
	...
	function __construct()
    {
        ...
        $this->load->model('mdl_contractor','mdl');
        $this->load->library('pagination');
    }
    
    public function unset_session_value() {

        $this->session->unset_userdata('contractor_s_name');
        $this->session->unset_userdata('contractor_s_phone');
        $this->session->unset_userdata('contractor_s_email');
        $this->session->unset_userdata('contractor_s_status');
        
        $this->session->unset_userdata('contractor_serach_data');
	...
	public function index() {
        ...
        $w .= 'AND customer_type = "contractor"';
        ...
        if($_POST) {
            ...
            if(isset($_POST['contractor_s_name']) AND $_POST['contractor_s_name'] != '') {
                $w .= " AND name like '%".$_POST['contractor_s_name']."%'";
                $this->session->set_userdata('contractor_s_name',$_POST['contractor_s_name']);
            }
            if(isset($_POST['contractor_s_phone']) AND $_POST['contractor_s_phone'] != '') {
                $w .= " AND mobile ='".$_POST['contractor_s_phone']."'";
                $this->session->set_userdata('contractor_s_phone',$_POST['contractor_s_phone']);
            }
            if(isset($_POST['contractor_s_email']) AND $_POST['contractor_s_email'] != '') {
                $w .= " AND email ='".$_POST['contractor_s_email']."'";
                $this->session->set_userdata('contractor_s_email',$_POST['contractor_s_email']);
            }
            if(isset($_POST['contractor_s_status']) AND $_POST['contractor_s_status'] != '') {
                $w .= " AND status ='".$_POST['contractor_s_status']."'";
                $this->session->set_userdata('contractor_s_status',$_POST['contractor_s_status']);
            }
            $_SESSION['contractor_serach_data'] = $w;
            $this->session->set_userdata('contractor_serach_data',$w);
        }
        
        if(isset($this->session->contractor_serach_data) AND $this->session->contractor_serach_data != ''){
             $w = $this->session->userdata('contractor_serach_data');
        }
        ...
        $config['base_url'] = site_url().'admin/contractor/index';
        $config['total_rows'] = $Record;
        $config['per_page'] = $paginationdata;
        $this->pagination->initialize($config);

	    $meta['page_title']                 = lang('Contractor');
        $this->data['rows']                 = $this->mdl->get_all_with_pagi('id',$config['per_page'],$this->uri->segment(3),$w);
        $this->data['states']               =  $this->maahi->get_states();
        $this->data['customers']               =  $this->mdl->get_customer();

    		$this->page_construct('contractor/view', $meta, $this->data);
	}

   public function add_row() {

        $d = ORM::for_table('sam_customers')->create();
        $d->customer_type         = 'contractor';
        $d->name                  = @$_POST['name'];
    ...
    public function edit($id) {
        $meta['page_title'] = lang('Edit Contractor');
        if($_POST && $_POST['name'] != '') {
            ...
            $this->page_construct('contractor/edit',$meta, $this->data);
        }
    }
  ...
  {
        $query = "SELECT * FROM `sam_customers` WHERE is_deleted = '0' AND customer_type = 'contractor' $w ORDER BY id DESC";
        $d = ORM::for_table('sam_customers')->raw_query($query)->find_array();
          
        $date =  date('d-m-Y');
        $excelName = 'contractor_'.$date;
    ...
    function get_details_inmodel()
    {
        ...
        $html = $this->load->view('/admin/views/contractor/view_detail',$this->data,true);
